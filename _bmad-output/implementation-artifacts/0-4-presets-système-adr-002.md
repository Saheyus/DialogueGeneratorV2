...
- [ ] Task 4: Créer composant PresetSelector.tsx (AC: #1, #2, #3, #5)
  - [ ] Créer `frontend/src/components/generation/PresetSelector.tsx`
  - [ ] Dropdown "📋 Charger preset ▼" avec liste presets (nom, icône, aperçu)
  - [ ] Bouton "💾 Sauvegarder preset..." ouvrant modal création
  - [ ] Modal création : Nom, icône emoji, aperçu lecture seule (personnages/lieux sélectionnés)
  - [ ] Menu contextuel : Renommer, Supprimer (AC: #5)
  - [ ] Pattern : Suivre style `GenerationOptionsModal.tsx` pour cohérence UI (overlay + header + contenu scrollable)
  - [ ] Cas limites : Liste vide → message "Aucun preset sauvegardé", tous refs invalides → warning modal avec option "Charger quand même"
  - [ ] Intégration : `usePresetStore` pour état global
  - [ ] Tests unitaires : Rendu dropdown, modal création, sélection preset
